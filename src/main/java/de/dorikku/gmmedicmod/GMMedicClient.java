...
public class GMMedicClient implements ClientModInitializer {
     ...
     */
    private static void registerBloodCommand(CommandDispatcher<FabricClientCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
        dispatcher.register(ClientCommandManager.literal("meb")
            .then(ClientCommandManager.argument("spieler", StringArgumentType.word()).executes(ctx -> {
                if (!ApiConnection.getInstance().isFeatureUnlocked()) {
                    ctx.getSource().sendFeedback(
                            Text.literal("[GM-Medic] Noch nicht verifiziert.").formatted(Formatting.RED)
                    );
                    return 0;
                }
                ...
                if (name == null) {
                    ctx.getSource().sendFeedback(
                            Text.literal("[GM-Medic] Ungültiger Spielername.").formatted(Formatting.RED)
                    );
                    return 0;
                }
                BloodChatNoteHandler.request(name);
                ctx.getSource().sendFeedback(
                        Text.literal("[GM-Medic] Frage Blutspende-Status von " + name + " ab...").formatted(Formatting.AQUA)
                );
                return 1;
            }))
            .executes(ctx -> {
                ctx.getSource().sendFeedback(Text.literal("§eNutze §f/meb <Spieler>"));
                return 1;
            })
        );
    }
